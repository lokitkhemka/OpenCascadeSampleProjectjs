 ...
 * current project into the `externalFiles` dictionary upon startup.*/
 function loadPrexistingExternalFiles(externalFileDict) {
  ...
  HandleMessage["loadPrexistingExternalFiles"] = loadPrexistingExternalFiles;
      ...
      loadFileSync(files[i]).then(async (fileText) => {
        ...
        if (i === files.length - 1) {
          if (lastImportedShape) {
            ...
            let response = HandleMessage["CombineAndRenderShapes"]({ maxDeviation: GUIState['MeshRes'] || 0.1 });
  ...
  HandleMessage["loadFiles"] = loadFiles;
  ...
  function importSTL(fileName, fileText) {
    ...
    if (reader.Read(readShape, fileName)) {
      ...
      console.log("Something in OCCT went wrong trying to read " + fileName + ".  \n" +
        "This project only imports small ASCII stl files for now!");
      return null;
    }
  }
  ...
  function saveShapeSTEP (filename = "OCCStepFile.step") {
  ...
  HandleMessage["saveShapeSTEP"] = saveShapeSTEP;
  
  /** Removes the externally imported shapes/files from the project. */ 
  HandleMessage["clearExternalFiles"] = () => {externalShapes = {}; };
