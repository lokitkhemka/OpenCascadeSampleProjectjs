var MainInterfaceLayout, MonacoEditor, threejsViewport,
...
function Initialize(ProjectContent = null) {
    ...
    if (ProjectContent) {
        ...
        if (MainInterfaceLayout != null) {
            MainInterfaceLayout.destroy();
            MainInterfaceLayout = null;
        }
        MainInterfaceLayout = new GoldenLayout(JSON.parse(ProjectContent));
    }
    else {
        let CodeStr = DefaultCode;
        GUIState = {};
        //Getting the code and guistate from the url
        if (loadFromURL) {
            codeStr  = decode(this.searchParams.get("code"));
            GUIState = JSON.parse(decode(this.searchParams.get("gui")));
        }
        //Default Golden Layout: Code on the left,
        //Model Output on the right, Conosle output on the bottom right
        MainInterfaceLayout = new GoldenLayout({
            content: [{
                type: 'row',
                content: [{
                    type: 'component',
    ...
    //Setup the Dockable Monaco Code Editor
    MainInterfaceLayout.registerComponent('CodeEditor', function (container, state) {
        MainInterfaceLayout.on("initialised", () => {
            if (MonacoEditor) {
            ...
            // Custom Functions Typescript Definitions...
            fetch(prefix + "/js/StandardLibraryIntellisense.ts").then((response) => {
                response.text().then(function (text) {
                    ExtraLibs.push({ content: text, filePath: 'file://' + prefix + '/js/StandardLibraryIntellisense.ts' });
            ...
            MonacoEditor.evaluateCode = (saveToURL = false) => {
                ...
                userGui = false;
                HandleMessage["addButton"]({ name: "Evaluate", label: "Function", callback: () => { MonacoEditor.evaluateCode(true) } });
                HandleMessage["addSlider"]({ name: "MeshRes", default: 0.1, min: 0.01, max: 2, step: 0.01, dp: 2 });
                HandleMessage["addCheckbox"]({ name: "Cache?", default: true });
                userGui = true;
                ...
                HandleMessage["saveFile"] = (payload) => {
    ...
    MainInterfaceLayout.registerComponent('cascadeView', function (container, state) {
        GUIState = state;
    ...
    MainInterfaceLayout.registerComponent('console', function (container) {
        ...
        if (!realConsoleLog) {
            ...
            HandleMessage["log"] = (payload) => { console.log(payload); };
            HandleMessage["error"] = (payload) => { window.workerWorking = false; console.error(payload); };
            ...
            HandleMessage["Progress"] = (payload) => {
     ...
     window.onorientationchange = function (event) {
        MainInterfaceLayout.updateSize(window.innerWidth, window.innerHeight -
            document.getElementsByClassName('topnav')[0].offsetHeight);
    };

    // Initialize the Layout
    MainInterfaceLayout.init();
    MainInterfaceLayout.updateSize(window.innerWidth, window.innerHeight -
    ...
    HandleMessage["addSlider"] = (payload) => {
    ...
    HandleMessage["addTextbox"] = (payload) => {
        if (!(payload.name in GUIState)) { GUIState[payload.name] = payload.default || ''; }
        addGuiSeparator();
    ...
    HandleMessage['addDropdown'] = (payload) => {
        if (!(payload.name in GUIState)) { GUIState[payload.name] = payload.default || ''; }
    ...
    HandleMessage["resetWorking"] = () => { window.workerWorking = false; }
}
 ...
 * into a `.json` file and saves it to the selected location. */
 async function SaveProject() {
    ...
    writeFile(file.handle, JSON.stringify(MainInterfaceLayout.toConfig(), null, 2)).then(() => {
        codeContainer.setTitle(file.handle.name);
...
function loadFiles(fileElementID = "files") {
    ...
    HandleMessage["loadFiles"] = (extFiles) => {
